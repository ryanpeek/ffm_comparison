...
# pick one:
(basefolder <- path_ext_remove(zips[grepl(pattern = version, zips)]))
...
# assuming files have same structure (BUT THEY DON'T!!!!)
# df <- bind_rows(lapply(glue("{basefolder}/{csv_files}"), read_csv))

# READ IN AND CONVERT TO LONG FORMAT --------------------------------------

# create a dataframe to map data into
dat_long <- tibble(filename = csv_files, version=version) %>%
...
write_csv(df_trim, file = glue("output/{fs::path_file(basefolder)}_tidy_df.csv.gz"))

# write a compressed .RData version
save(df_trim, file = glue("output/{fs::path_file(basefolder)}_tidy_df.rda"))
